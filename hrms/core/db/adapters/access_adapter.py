...
class AccessAdapter(DBAdapter):
    ...
    def _create_tables_if_not_exists(self):
        ...
        try:
            ...
            raise DatabaseConnectionError(f"創建資料庫表格時發生錯誤: {str(e)}")
    ...
    def get_connection(self):
        ...
        try:
            ...
            raise DatabaseConnectionError(f"無法連接到 Access 資料庫: {str(e)}")
    ...
    def list(self, table: str, filters: Optional[Dict[str, str]] = None, limit: Optional[int] = None) -> List[Dict]:
        try:
            ...
            raise DatabaseConnectionError(f"執行查詢時發生錯誤: {str(e)}")
    ...
    def upsert(self, table: str, pk: str, row: Dict) -> Dict:
        try:
            ...
            if "duplicate" in str(e).lower() or "primary key" in str(e).lower():
                raise DuplicateRecordError(f"記錄已存在: {pk} = {row[pk]}")
            else:
